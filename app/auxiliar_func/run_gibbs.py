...
class RunGibbs:
    ...
    def run_gibbs(self, progress_callback=None):
        solver = Gibbs(
            ...
            for P in P_vals:
                if ref_idx is not None and n_vals is not None:
                    for n in n_vals:
                        ...
                        try:
                            result = solver.solve_gibbs(initial_copy, T, P)
                        except Exception as e:
                            warnings.warn(f"Gibbs falhou em T={T:.1f}, P={P:.3f}, n={n:.4f}: {e}")
                            result = nan_row
                        row = {comp: round(val, 3) for comp, val in zip(self.components, result)}
                        row[self.components[ref_idx] + ' Initial'] = n
                        row.update({'Temperature': T, 'Pressure': P})
                    ...
                    try:
                        result = solver.solve_gibbs(self.initial.copy(), T, P)
                    except Exception as e:
                        warnings.warn(f"Gibbs falhou em T={T:.1f}, P={P:.3f}: {e}")
                        result = nan_row
                    row = {comp: round(val, 3) for comp, val in zip(self.components, result)}
                    row.update({'Temperature': T, 'Pressure': P})
                    result_list.append(row)
