...
_PROGRESS_BAR_STYLE = """
    QProgressBar {
        ...
        background: #f0f0f0; text-align: center; color: black; font-size: 10px;
    }
    QProgressBar::chunk {
...
class MaxS(QWidget):
    ...
    def run_entropy(self):
        ...
        n_iter = self.n_component_values if (self.reference_componente and self.reference_componente != '---') else 1
        total = self.n_temperature * self.n_pressure * n_iter
        self._start_simulation(total)

        self._worker = SimWorker(runner.run_entropy)
        self._worker.progress.connect(self._on_progress)
        self._worker.result_ready.connect(self._on_entropy_done)
    ...
    def _on_entropy_done(self, results):
        self._cleanup()
        ...
        msg.setText("A simulação de Entropia foi concluída com sucesso!")
    ...
    def _section1(self):
        ...
        """)
        self.btn_run.clicked.connect(self.run_entropy)
        ...
        col3 = QVBoxLayout()
        lbl = QLabel()
        px = QPixmap(resource_path("app/imgs/maxS.png"))
